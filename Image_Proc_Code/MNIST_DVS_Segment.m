...
% @编写：wtx
% @更新日期：2022/1/1
% @运行时间：不Resize时'None'与'Compact'28s，'Linear'76s
% @概述：采用硬分段的方式，将DVS数据集基于事件个数分段，时间压缩方式支持“不压缩(None)”、“紧凑型(Compact)”和“线性(Linear)”三种方式，可根据实际情况选择
% @输入数据格式：原始mat文件中包含10000个cell，每个cell表示一张图像的脉冲，其第1、4、5列分别表示时间、像素横坐标、像素纵坐标，其它列数据无用
...
% @备注8：该数据集虽然标的是100ms，但实际上并非每张图都在100ms范围内，软件处理时应注意丢掉窗口外的事件
% -----------------------------------------------------------------------------------------------------------------------------------------------------%
clear;clc;
load('.\Definition_pkg');
load('..\..\Common_Datasets\DVS\MNIST_DVS\Original_mat\MNIST_DVS_28x28_10000_100ms');
%----------------------------------------------------------------------参数配置区-------------------------------------------------------------------%
%训练集占比
TRAIN_RATIO = 0.8; 

%分段参数
NEVENT_PER_SEGMENT = 300;  %每个Segment的事件数
...
%注1：由28x28Resize成16x16就是按照时间步，依次将图像padding成32x32，然后做maxpooling到16x16，对于“不压缩(None)”和“紧凑型(Compact)”，由于每个时间步就1个事件，
%     因此不必真的对二维矩阵做maxpooling，直接对坐标x, y进行映射即可，即16x16_x = ceil((28x28_x + 2) / 2)，16x16_y = ceil((28x28_y + 2) / 2)
...
SAVE_DIR = strcat('..\..\Common_Datasets\DVS\MNIST_DVS\', folder_name, '\');
...
%打乱原始数据
%返回一行从1到1w的整数中的1w个，而且这1w个数各不相同
rand_num = randperm(MNIST_DVS_NCELL, MNIST_DVS_NCELL); 
CINs_rand = cell(1, MNIST_DVS_NCELL); %打乱后的数据
for i = 1 : (MNIST_DVS_NCELL)
    CINs_rand{i} = CINs{rand_num(i)};
end

%分段训练集数据，并分配好标签
cnt = 0;
for i = 1 : round(MNIST_DVS_NCELL * TRAIN_RATIO)
    ...
    for j = 1 : img_segment_num
        for k = 1 : NEVENT_PER_SEGMENT
            ...
            if (strcmp(TIME_COMP_MODE, 'Linear'))
                %addr = TriOp(IF_RESIZE, (ceil((x+2)/2) - 1) * DVS_RESIZE_SIZE + ceil((y+2)/2), (x - 1) * MNIST_SIZE + y);
                addr = TriOp(IF_RESIZE, (ceil((x+2)/2) - 1) * DVS_RESIZE_SIZE + ceil((y+2)/2), x* MNIST_SIZE + (y+1)); %由于 x y坐标下标从0开始
                if addr < 0
                    fprintf("addr < 0!!");
                    x
                    y
...
%分段测试集数据，并分配好标签
cnt = 0;
for i = round(MNIST_DVS_NCELL * TRAIN_RATIO) + 1 : MNIST_DVS_NCELL
    img_spike_num = length(CINs_rand{i}(:, 1));                  %该张图像脉冲总数
    img_segment_num = floor(img_spike_num / NEVENT_PER_SEGMENT); %该张图像分段后段数
    t_pre = CINs_rand{i}(1, 1);  %确保每张图都从0时刻开始
    t_compact = 0;
    ...
    for j = 1 : img_segment_num
        for k = 1 : NEVENT_PER_SEGMENT
            ...
            if (strcmp(TIME_COMP_MODE, 'Linear'))
                %addr = TriOp(IF_RESIZE, (ceil((x+2)/2) - 1) * DVS_RESIZE_SIZE + ceil((y+2)/2), (x - 1) * MNIST_SIZE + y);
                addr = TriOp(IF_RESIZE, (ceil((x+2)/2) - 1) * DVS_RESIZE_SIZE + ceil((y+2)/2), x * MNIST_SIZE + (y+1));
                t_linear_addr_temp = [t_linear_addr_temp addr];  %数组元素个数超过1000时不建议采用这种方式添加元素，应提前分配内存
                if (t_linear_post ~=  t_linear) %当前时间步事件地址均添加完毕
                    t_linear_addr_temp = unique(t_linear_addr_temp);  %去除相同的地址
                    len = length(t_linear_addr_temp); %获取事件个数
