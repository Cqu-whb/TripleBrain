...
% @输入数据格式：原始mat文件中包含100个cell，每个cell表示一张图像的脉冲，其第1、4、5列分别表示时间、像素横坐标、像素纵坐标，每张图约2s或4s,标签是0-2
% @输出格式：1行n列数组[addr1 t1 addr2 t2 ... -1 addr1 t1 addr2 t2 ... -1 ...], 地址addr范围：[1, img_size^2], 时间戳t范围：[0, ...]，标签0-2
...
%打乱原始数据
%返回一行从1到484的整数中的484个，而且这484个数各不相同  ---->改动 1w - 484
rand_num = randperm(POSTURE_DVS_NCELL, POSTURE_DVS_NCELL);  
CINs_rand = cell(1, POSTURE_DVS_NCELL); %打乱后的数据
    ...
    for j = 1 : img_segment_num
        for k = 1 : NEVENT_PER_SEGMENT
            ...
            if (strcmp(TIME_COMP_MODE, 'None'))
                cnt = cnt + 1;
                train_coding(cnt) = TriOp(IF_RESIZE, (ceil(x/2) - 1) * DVS_RESIZE_SIZE + ceil(y/2), (x - 1) * POSTURE_SIZE + y); %地址
                cnt = cnt + 1;
...
for i = round(POSTURE_DVS_NCELL * TRAIN_RATIO) + 1 : POSTURE_DVS_NCELL
    ...
    for j = 1 : img_segment_num
        for k = 1 : NEVENT_PER_SEGMENT
            ...
            if (strcmp(TIME_COMP_MODE, 'None'))
                cnt = cnt + 1;
                test_coding(cnt) = TriOp(IF_RESIZE, (ceil(x/2) - 1) * DVS_RESIZE_SIZE + ceil(y/2), (x - 1) * POSTURE_SIZE + y); %地址
                cnt = cnt + 1;
