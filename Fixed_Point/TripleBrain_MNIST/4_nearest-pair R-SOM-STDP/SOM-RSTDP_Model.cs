...
namespace SOM_RSTDPModel
{
    ...
    {
        public static int RSTDP_EP = 4;//orl 15 mnist 15 yale 20 r-som-stdp mnist 2 
        ...
        */
                            ...
                            else if (LearnRule == "R_STDP")
                            {
                                for (int j = 0; j < Parameter.InputLayerNeurons; j++)
                                {
                                    ...
                                    {   //som-stdp&rstdp : 8.25  som-r-stdp:0.2 
                                        ...
                                        Parameter.WeightTensor[i, j] -= (((((int)Math.Round(0.2 *  (1 << 10)) * Parameter.PositiveLR) >> 10) * ((PreSpikeTrace[j] * Parameter.PreExpTable[delta_time]) >> 4 << 6) >> 10) << 2);
                                    }
                                    if (Parameter.WeightTensor[i, j] > Parameter.WeightUpperBound)
                                        Parameter.WeightTensor[i, j] = Parameter.WeightUpperBound;
                                    else if (Parameter.WeightTensor[i, j] < Parameter.WeightLowerBound)
        ...
        public static void SOMTrain(string pattern)
        {
            ...
            for (int externEp = 0; externEp < 1; externEp++) //mnist 1 orl 100 yale 50
            {
                for (int Epoch = 0; Epoch < MAX_EPOCH; Epoch++)
                {
                    ...
                    for (int i = 0; i < TrainNum; i++)
                    {
                        ...
                        //if(TrainLabel[imgNum] == 0|| TrainLabel[imgNum] == 1|| TrainLabel[imgNum] == 2|| TrainLabel[imgNum] == 3)  
            ...
            //动态阈值衰减
            DynamicThreasholdScaling(64);  //mnist 64
            //记录动态阈值
