...
namespace SOM_RSTDPModel
{
    ...
    {
        public static int RSTDP_EP = 15;//orl 15 mnist 15 yale 20 r-som-stdp mnist 2 
        ...
        */
                        ...
                        if (output_fire_flag[i] == true)
                        {
                            ...
                            else if (LearnRule == "R_STDP")
                            {
                                for (int j = 0; j < Parameter.InputLayerNeurons; j++)
                                {
                                    ...
                                    {   //som-stdp&rstdp : 1.0   som-r-stdp: 0.5
                                        Parameter.WeightTensor[i, j] += 1.0 * Parameter.PositiveLR * (PreSpikeTrace[j] * Parameter.PreExpTable[delta_time]);  
                                    }
                                    else
                                    {   //som-stdp&rstdp : 8.25  som-r-stdp:0.2 
                                        Parameter.WeightTensor[i, j] -= 8.25 * Parameter.PositiveLR * (PreSpikeTrace[j] * Parameter.PreExpTable[delta_time]);
                                    }
                                    if (Parameter.WeightTensor[i, j] > Parameter.WeightUpperBound)
                                        Parameter.WeightTensor[i, j] = Parameter.WeightUpperBound;
                                    else if (Parameter.WeightTensor[i, j] < Parameter.WeightLowerBound)
                ...
                if (EvenSpikeNum < (int)(ImageData.Length / 2))
				{
					...
					else if (LearnRule == "R_STDP")
					{
						for (int i = 0; i < Parameter.OutputLayerNeurons; i++)
						{
                            ...
                            //som-stdp&rstdp mnist: 1.0  som-r-stdp:1.0 
                            if (InputClassLabel == Parameter.NeuronRelativeLabel[i])
                                Parameter.WeightTensor[i, ImageData[2 * EvenSpikeNum + 1]] -= 1.0 * Parameter.NegativeLR * (PostSpikeTrace[i] * Parameter.PostExpTable[delta_time]);
                            //som-stdp&rstdp mnist: 1.0 som-r-stdp:1.0 
                            else
                                Parameter.WeightTensor[i, ImageData[2 * EvenSpikeNum + 1]] += 1.0 * Parameter.NegativeLR * (PostSpikeTrace[i] * Parameter.PostExpTable[delta_time]);
							if (Parameter.WeightTensor[i, ImageData[2 * EvenSpikeNum + 1]] > Parameter.WeightUpperBound)
        ...
        public static void SOMTrain(string pattern)
        {
            ...
            //动态阈值衰减
            DynamicThreasholdScaling(64 / 256.0);  //mnist 64
            //记录动态阈值
